...
class MonoQPD(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        l_features = self.feature_converter(l_features)
        l_features = l_features[::-1] # Reverse the order of the features

        if test_mode:
            original_disp, upsampled = self.qpdnet(l_features, image1, image2, iters=iters, test_mode=test_mode, flow_init=None)
            return original_disp, upsampled
        else:
            disp_predictions = self.qpdnet(l_features, image1, image2, iters=iters, test_mode=test_mode, flow_init=None)
