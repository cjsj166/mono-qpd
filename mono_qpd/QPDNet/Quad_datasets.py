...
# Counter for saving intermediate augmentation results
_INTER_RESULT_COUNT = 0
_INTER_RESULT_MAX = 50


def _save_inter_result(items, idx):
    out_dir = Path("inter-result")
    out_dir.mkdir(exist_ok=True)

    center = items["center"]
    cv2.imwrite(
        str(out_dir / f"{idx:04d}_center.png"), cv2.cvtColor(center, cv2.COLOR_RGB2BGR)
    )

    for j, lrtb_img in enumerate(items["lrtb_list"]):
        label = ["left", "right", "top", "bottom"][j] if j < 4 else f"lrtb{j}"
        cv2.imwrite(
            str(out_dir / f"{idx:04d}_{label}.png"),
            cv2.cvtColor(lrtb_img, cv2.COLOR_RGB2BGR),
        )

    if "disp" in items:
        disp = items["disp"][..., 0]
        disp_vis = disp - disp.min()
        dmax = disp_vis.max()
        if dmax > 0:
            disp_vis = (disp_vis / dmax * 255).astype(np.uint8)
        else:
            disp_vis = np.zeros_like(disp, dtype=np.uint8)
        disp_vis = cv2.applyColorMap(disp_vis, cv2.COLORMAP_INFERNO)
        cv2.imwrite(str(out_dir / f"{idx:04d}_disp.png"), disp_vis)

    if "AiF" in items:
        cv2.imwrite(
            str(out_dir / f"{idx:04d}_AiF.png"),
            cv2.cvtColor(items["AiF"], cv2.COLOR_RGB2BGR),
        )  # Save at most this many samples
...
class QuadDataset(data.Dataset):
    ...
    def preprocess(self, items, crop_h, crop_w, resize_h, resize_w):
        ...
        image_list = list(
            map(lambda x: self._resize(x, resize_h, resize_w), image_list)
        )
    ...
    def __getitem__(self, index):
        ...
        if self.augmentor is not None:
            ...
            # global _INTER_RESULT_COUNT
            # if _INTER_RESULT_COUNT < _INTER_RESULT_MAX:
            #     _save_inter_result(items, _INTER_RESULT_COUNT)
            #     _INTER_RESULT_COUNT += 1

        # Convert numpy to torch tensor
        items["lrtb_list"] = np.stack(
            items["lrtb_list"][0 : self.lrtb_image_num], axis=-1
        )
        items["center"] = (
            torch.from_numpy(items["center"]).permute(2, 0, 1).float()
        )  # 3 x h x w
        items["lrtb_list"] = (
            torch.from_numpy(items["lrtb_list"]).permute(3, 2, 0, 1).float()
        )  # image_num x 3 x h x w
        if "disp" in self.gt_types:
            items["disp"] = (
                torch.from_numpy(items["disp"]).permute(2, 0, 1).float()
            )  # 2 x h x w
        if "inv_depth" in self.gt_types:
            items["inv_depth"] = (
                torch.from_numpy(items["inv_depth"]).permute(2, 0, 1).float()
            )
        if "AiF" in self.gt_types:
        ...
        # Add validity mask
        if "disp" in self.gt_types:
            items["disp_valid"] = torch.ones(
                1, items["disp"].shape[-2], items["disp"].shape[-1]
            ).float()
        if "inv_depth" in self.gt_types:
            items["inv_depth_valid"] = torch.ones(
                1, items["inv_depth"].shape[-2], items["inv_depth"].shape[-1]
            ).float()
...
class QPD(QuadDataset):
    def __init__(
        ...
        imagel_list = sorted(
            glob(os.path.join(root, image_set + "_l", "source", "seq_*/*.png"))
        )
        imager_list = sorted(
            glob(os.path.join(root, image_set + "_r", "source", "seq_*/*.png"))
        )
        imaget_list = sorted(
            glob(os.path.join(root, image_set + "_t", "source", "seq_*/*.png"))
        )
        imageb_list = sorted(
            glob(os.path.join(root, image_set + "_b", "source", "seq_*/*.png"))
        )
        imagec_list = sorted(
            glob(os.path.join(root, image_set + "_c", "source", "seq_*/*.png"))
        )
        aif_list = sorted(
            glob(os.path.join(root, image_set + "_c", "target", "seq_*/*.png"))
        )
        disp_list = sorted(
            glob(os.path.join(root, image_set + "_c", "target_disp", "seq_*/*.npy"))
        )

        if datatype == "dual":
            for idx, (imgc, imgl, imgr) in enumerate(
                zip(imagec_list, imagel_list, imager_list)
            ):
                self.image_list += [[imgc, imgl, imgr]]
        elif datatype == "quad":
            for idx, (imgc, imgl, imgr, imgt, imgb) in enumerate(
                zip(imagec_list, imagel_list, imager_list, imaget_list, imageb_list)
            ):
...
class QPD_AiF(QuadDataset):
    def __init__(
        ...
        imagec_list = sorted(
            glob(os.path.join(root, image_set + "_c", "target", "seq_*/*.png"))
        )  # center image is replaced by all-in-foucs image
        disp_list = sorted(
            glob(os.path.join(root, image_set + "_c", "target_disp", "seq_*/*.npy"))
        )
...
class QPD_QPDv2(QuadDataset):
    def __init__(
        ...
        qpd_imagel_list = glob(
            os.path.join(qpd_root, image_set + "_l", "source", "seq_*/*.png")
        )
        qpdv2_imagel_list = glob(
            os.path.join(qpdv2_root, image_set + "_l", "source", "seq_*/*.png")
        )
        imagel_list = sorted(qpd_imagel_list + qpdv2_imagel_list)

        qpd_imager_list = sorted(
            glob(os.path.join(qpd_root, image_set + "_r", "source", "seq_*/*.png"))
        )
        qpdv2_imager_list = sorted(
            glob(os.path.join(qpdv2_root, image_set + "_r", "source", "seq_*/*.png"))
        )
        imager_list = sorted(qpd_imager_list + qpdv2_imager_list)

        qpd_imaget_list = sorted(
            glob(os.path.join(qpd_root, image_set + "_t", "source", "seq_*/*.png"))
        )
        qpdv2_imaget_list = sorted(
            glob(os.path.join(qpdv2_root, image_set + "_t", "source", "seq_*/*.png"))
        )
        imaget_list = sorted(qpd_imaget_list + qpdv2_imaget_list)

        qpd_imageb_list = sorted(
            glob(os.path.join(qpd_root, image_set + "_b", "source", "seq_*/*.png"))
        )
        qpdv2_imageb_list = sorted(
            glob(os.path.join(qpdv2_root, image_set + "_b", "source", "seq_*/*.png"))
        )
        imageb_list = sorted(qpd_imageb_list + qpdv2_imageb_list)

        qpd_imagec_list = sorted(
            glob(os.path.join(qpd_root, image_set + "_c", "source", "seq_*/*.png"))
        )
        qpdv2_imagec_list = sorted(
            glob(os.path.join(qpdv2_root, image_set + "_c", "source", "seq_*/*.png"))
        )
        imagec_list = sorted(qpd_imagec_list + qpdv2_imagec_list)

        qpd_aif_list = sorted(
            glob(os.path.join(qpd_root, image_set + "_c", "target", "seq_*/*.png"))
        )
        qpdv2_aif_list = sorted(
            glob(os.path.join(qpdv2_root, image_set + "_c", "target", "seq_*/*.png"))
        )
        aif_list = sorted(qpd_aif_list + qpdv2_aif_list)

        qpd_disp_list = sorted(
            glob(os.path.join(qpd_root, image_set + "_c", "target_disp", "seq_*/*.npy"))
        )
        qpdv2_disp_list = sorted(
            glob(
                os.path.join(qpdv2_root, image_set + "_c", "target_disp", "seq_*/*.npy")
            )
        )
...
class DP5K(QuadDataset):
    def __init__(
        ...
        disp_list = sorted(
            glob(os.path.join(root, image_set + "_c", "target_depth", "seq_*/*.png"))
        )

        for idx, (imgc, imgl, imgr, disp, aif) in enumerate(
            zip(imagec_list, imagel_list, imager_list, disp_list, aif_list)
        ):
            if datatype == "dual":
                self.image_list += [[imgc, imgl, imgr]]
            if "disp" in gt_types:
...
class DP119(QuadDataset):
    def __init__(
        ...
        assert os.path.exists(root)
        imagel_list = sorted(
            glob(os.path.join(root, image_set + "_l", "source", "seq_*/*.jpg"))
        )
        imager_list = sorted(
            glob(os.path.join(root, image_set + "_r", "source", "seq_*/*.jpg"))
        )
        imagec_list = sorted(
            glob(os.path.join(root, image_set + "_c", "source", "seq_*/*.jpg"))
        )
        depth_list = sorted(
            glob(os.path.join(root, image_set + "_c", "target_depth", "seq_*/*.npy"))
        )

        for idx, (imgc, imgl, imgr, depth) in enumerate(
            zip(imagec_list, imagel_list, imager_list, depth_list)
        ):
            self.image_list += [[imgc, imgl, imgr]]
...
class DDDP(QuadDataset):
    def __init__(
        ...
        imagel_list = sorted(
            glob(os.path.join(root, image_set + "_l", "source", "*.png"))
        )
        imager_list = sorted(
            glob(os.path.join(root, image_set + "_r", "source", "*.png"))
        )
        imagec_list = sorted(
            glob(os.path.join(root, image_set + "_c", "source", "*.png"))
        )
        aif_list = sorted(glob(os.path.join(root, image_set + "_c", "target", "*.png")))

        for idx, (imgc, imgl, imgr, aif) in enumerate(
            zip(imagec_list, imagel_list, imager_list, aif_list)
        ):
            if datatype == "dual":
                self.image_list += [[imgc, imgl, imgr]]
            elif datatype == "quad":
...
class Real_QPD(QuadDataset):
    def __init__(
        ...
        imagel_list = sorted(
            glob(
                os.path.join(
                    root, "**", "scale3", image_set + "_l", "source", "seq_*/*.png"
                )
            )
        )
        imager_list = sorted(
            glob(
                os.path.join(
                    root, "**", "scale3", image_set + "_r", "source", "seq_*/*.png"
                )
            )
        )
        ...
        imaget_list = sorted(
            glob(
                os.path.join(
                    root, "**", "scale3", image_set + "_t", "source", "seq_*/*.png"
                )
            )
        )
        imageb_list = sorted(
            glob(
                os.path.join(
                    root, "**", "scale3", image_set + "_b", "source", "seq_*/*.png"
                )
            )
        )
        ...
        imagec_list = sorted(
            glob(
                os.path.join(
                    root, "**", "scale3", image_set + "_c", "source", "seq_*/*.png"
                )
            )
        )
...
class DPD_Disp(QuadDataset):
    def __init__(
        ...
        depth_list = sorted(
            glob(os.path.join(root, image_set + "_c", "target_depth", "seq_*/*.TIF"))
        )
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        if dataset_name == "QPD_QPDv2":
            new_dataset = QPD_QPDv2(
                datatype=args.datatype,
                gt_types=args.qpd_gt_types,
                aug_params=aug_params,
                qpd_root="datasets/QP-Data",
                qpdv2_root="datasets/QP-Data-v2",
            )
