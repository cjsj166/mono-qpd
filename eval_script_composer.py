...
if __name__ == "__main__":
    ...
    script = []
    with Path(args.exp_name).open() as f:
        script = f.readlines()
    ...
    if args.epoch:
        ...
        job_size = max(1, len(ckpts) // args.job_num)
        job_scripts = []
        ...
        for i in range(0, len(ckpts), job_size):
            ...
            script[-1] = script[-1] + f" --ckpt_min_epoch {job_min_epoch} --ckpt_max_epoch {job_max_epoch}"
            with script_path.open("w") as f:
                f.write(script)
        
        print("; ".join([f"cd {save_dir}; sub {script_path.name}" for script_path in job_scripts]))
